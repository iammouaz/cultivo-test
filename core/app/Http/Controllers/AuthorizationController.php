...
class AuthorizationController extends Controller
{
    ...
    public function authorizeForm()
    {
        ...
        if (auth()->check()) {
            ...
            if (!$user->status) {
                ...
                $pageTitle = __('Email verification form');
    ...
    public function sendVerifyCode(Request $request)
    {
        ...
        if ($request->type === 'email') {
            ...
            $notify[] = ['success', __('Email verification code sent successfully')];
            return back()->withNotify($notify);
        } elseif ($request->type === 'phone') {
            sendSms($user, 'SVER_CODE', [
                'code' => $user->ver_code
            ]);
            $notify[] = ['success', __('SMS verification code sent successfully')];
            return back()->withNotify($notify);
        } else {
            throw ValidationException::withMessages(['resend' => __('Sending Failed')]);
        }
    }
    ...
    public function emailVerification(Request $request)
    {
        ...
        throw ValidationException::withMessages(['email_verified_code' => __('Verification code didn\'t match!')]);
    }
    ...
    public function smsVerification(Request $request)
    {
        ...
        throw ValidationException::withMessages(['sms_verified_code' => __('Verification code didn\'t match!')]);
    }
