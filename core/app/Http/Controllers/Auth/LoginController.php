...
//        }
        if (!is_null($login_type) && $login_type == "ace") {//add other login type logic here
            $pageTitle = "Sign In Ace Memeber";
        }
        else{
            $pageTitle = "Sign In";
        }
        return view(activeTemplate() . 'user.auth.login', compact('pageTitle','login_type'));
    }
    ...
    public function login(Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            if(config('app.COMMERCE_MODE')==1){
                $user_temp = User::where('email', $request->input('username'))
        ...
        //ace event login
        $login_type = LoginController::LOGIN_ACE_MEMBER;
        $user_temp = User::where('email', $request->input('username'))
            ->orWhere('username', $request->input('username'))->first();


        $is_email = filter_var($request->username, FILTER_VALIDATE_EMAIL);
        $data = WordPressSiteAuthHelper::check($request->input('username'), $request->input('password'), $is_email);
        ...
        if (!$user_temp && $login_type == LoginController::LOGIN_ACE_MEMBER && config('app.enable_ace_login')) {//ace event login
