    ...
    <div class="row">
        <div class="col-12">
            <div class="mb-30 d-flex align-items-center justify-content-between">
                <div>{{ $email_template->name }}</div>
                <div>
                    <button type="submit" class="btn btn-block btn--primary mr-2">
                        @include('templates.basic.svgIcons.save')
                        @lang('Save')
                    </button>
                </div>
            </div>
                            ...
                            <tbody class="list">
                                @forelse($email_template->shortcodes as $shortcode => $key)
                                    <tr>
                                        ...
                                        <td data-label="@lang('Description')">{{ $key }}</td>
                                    </tr>
                                @empty
                                    <tr>
        ...
        <div class="col-md-12">
            <div class="card mt-5">
                <div class="card-header bg--dark">
                    <h5 class="card-title text-white">{{ $pageTitle }}</h5>
                </div>
                    @csrf
                    <div class="card-body">
                        <div class="form-row">
                            @if ($pageTitle == 'Bid Winner')
                                ...
                                <div class="form-group col-md-4">
                                    <label class="font-weight-bold">@lang('CC')</label>
                                    <input type="email" class="form-control form-control-lg"
                                        placeholder="@lang('CC')" name="cc" value="{{ $email_template->cc }}"
                                        multiple />
                                </div>
                                <div class="form-group col-md-4">
                            ...
                            @else
                                ...
                                <div class="form-group col-md-6">
                                    <label class="font-weight-bold">@lang('CC') <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control form-control-lg"
                                        placeholder="@lang('CC')" name="cc" value="{{ $email_template->cc }}" />
                                </div>
