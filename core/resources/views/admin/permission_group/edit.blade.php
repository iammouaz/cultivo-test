...
@section('panel')
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <form action="{{ route('admin.permissiongroup.update', $permissiongroup->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                                    ...
                                    <div class="row mb-none-15">
                                        <div class="col-sm-12 col-xl-4 col-lg-6 mb-15">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control " placeholder="@lang('Group Name')" name="name" value="{{ $permissiongroup->name_group }}" required/>
                                            </div>
                                        </div>
                                        <?php
                                            foreach($permissiongroup->event as $event){$events_selected[] = $event->id ;}
                                        ?>
                                        ...
                                        <?php
                                            foreach($permissiongroup->users as $user){$users_selected[] = $user->id ;}
                                            foreach($permissiongroup->products as $product){$products_selected[] = $product->id;}
                                        ?>
                                        <div class="col-sm-12 col-xl-4 col-lg-6 mb-15">
                                        ...
                                        <div class="col-sm-12 col-xl-4 col-lg-6 mb-15">
                                            <div class="form-group">
                                                ...
                                                <select id="users" name="users_id[]" class="form-control" required multiple>
                                                    @foreach ($users as $user)
                                                        <option value="{{ $user->id }}" <?php if(in_array($user->id,$users_selected)){echo 'Selected';} ?> >{{ $user->firstname .' '. $user->lastname }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-sm-12 col-xl-4 col-lg-6 mb-15">
                                            <div class="form-group">
                                                ...
                                                <select name="status" class="form-control" required >
                                                        <option value="1" {{ $permissiongroup->status == 1 ? 'Selected':'' }}>Active</option>
                                                        <option value="0" {{ $permissiongroup->status == 0 ? 'Selected':'' }}>Pending</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.permissiongroup.index') }}" class="btn btn-sm btn--primary box--shadow1 text--small"><i class="la la-fw la-backward"></i> @lang('Go Back') </a>
@endpush
