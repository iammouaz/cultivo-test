...
<body>
    ...
    <script>
        ...
        const custom_font_list = @json(App\Models\SiteThemeSetting::where('name', 'like', '%family')->get());


        var nicEditorFontFamilySelect = nicEditorSelect.extend({
            init: function() {
                this.setDisplay("Font&nbsp;Family...");
                custom_font_list.forEach((item) => {
                    addFont(item.name, item.value)
                    const label = item.name.replace("family", "font").replace(/_/g, " ")
                    this.add(`${item.name}`, `<font face="${item.name}">${label}</font>`);
                })

                // this.add('comic sans ms', '<font face="Comic Sans MS">Comic Sans</font>');
                // this.add('courier new', '<font face="Courier New">Courier New</font>');
                // Add your custom font here
                // this.add('mycustomfont', '<font face="MyCustomFont">My Custom Font</font>');
            }
        });
        ...
        // Function to add a font to the DOM
