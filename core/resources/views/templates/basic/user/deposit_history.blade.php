        ...
        @if(count($logs) >0)
            @foreach($logs as $k=>$data)
                <tr>
                    ...
                    <td data-label="@lang('Amount')"><strong>{{showAmount($data->amount)}} {{$general->cur_text}}</strong></td>
                    ...
                    <td data-label="@lang('MORE')">
                        <div>
                            ...
                            <button class="btn btn-sm btn--base approveBtn"
                                data-info="{{ $details }}"
                                data-id="{{ $data->id }}"
                                data-amount="{{ showAmount($data->amount)}} {{ $general->cur_text }}"
                                data-charge="{{ showAmount($data->charge)}} {{ $general->cur_text }}"
                                data-after_charge="{{ showAmount($data->amount + $data->charge)}} {{ $general->cur_text }}"
                                data-rate="{{ showAmount($data->rate)}} {{ $data->method_currency }}"
                                data-payable="{{ showAmount($data->final_amo)}} {{ $data->method_currency }}"
