    ...
    <div class="container">
        <div class="header-wrapper d-flex flex-wrap align-items-center justify-content-between ">
            ...
            <div class="menu-area">
                ...
                <a class="menu-logo" href="{{ getHomePageUrl() }}">
                    <img style="max-width: 100px" id="logo-mobile" alt="M Cultivo logo">
                </a>
                @if (config('app.APP_SHOW_MESSAGE', false))
                    <strong>{{ config('app.APP_MESSAGE') }}</strong>
                @endif
                ...
                <div class="panel-group" id="accordion">
                    @if (getNewAuctionId() && getNewAuctionLabel() && ($event = \App\Models\Event::find(getNewAuctionId())) != null)
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <h4 class="panel-title pe-4 ps-18"><a href="{{ route('event.details', [$event->id, slug($event->name)]) }}">{{ getNewAuctionLabel() }}</a> </h4>
                            </div>
                        </div>
                    @endif
                    @if (getPastAuctionLabel())
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <h4 class="panel-title pe-4 ps-18"> <a data-toggle="collapse" data-parent="#accordion" href="#navbarDropdownPastAuctionsMobile">{{ getPastAuctionLabel() }}<span class="caret"></span></a> </h4>
                            </div>
                            <!-- Note: By adding "in" after "collapse", it starts with that particular panel open by default; remove if you want them all collapsed by default -->
                            <div id="navbarDropdownPastAuctionsMobile" class="panel-collapse collapse in">
                                <ul class="list-group">
                                    @foreach (\App\Models\Event::whereIn('id', getPastAuctionIds())->get() as $event)
                                        <li>
                                            <h5 class="panel-title pe-4 ps-20"><a href="{{ route('event.details', [$event->id, slug($event->name)]) }}">{{ $event->name }}</a></h5>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                    ...
                    @foreach (is_array(getNavbarLinks()) ? getNavbarLinks() : [] as $link)
                        @if (!($link['is_menu'] ?? null))
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h4 class="panel-title pe-4 ps-18"><a href="{{ $link['with_iframe'] ?? 'off' == 'on' ? route('external', ['link' => $link['url']]) : $link['url'] }}">{{ $link['label'] ?? '' }}</a> </h4>
                                </div>
                            </div>
                        @else
                            <div class="panel panel-default">
                                <div class="panel-heading">
                                    <h4 class="panel-title pe-4 ps-18"> <a data-toggle="collapse" data-parent="#accordion" href="#navbarDropdownMobile{{ $loop->index }}">{{ $link['label'] ?? '' }}<span class="caret"></span></a> </h4>
                                </div>
                                <!-- Note: By adding "in" after "collapse", it starts with that particular panel open by default; remove if you want them all collapsed by default -->
                                <div id="navbarDropdownMobile{{ $loop->index }}" class="panel-collapse collapse in">
                                    <ul class="list-group">
                                        @foreach (isset($link['menu_items']) && is_array($link['menu_items']) ? $link['menu_items'] : [] as $subMenu)
                                            <li>
                                                <h5 class="panel-title pe-4 ps-20"><a href="{{ $subMenu['with_iframe'] ?? 'off' == 'on' ? route('external', ['link' => $subMenu['url']]) : $subMenu['url'] }}">{{ $subMenu['label'] ?? '' }}</a></h5>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                ...
                <div class="d-flex d-lg-none mt-4 justify-content-center">
                    ...
                    @else
                    <a class="login-link" href="{{ route('user.home') }}"
                        style="padding: 0px;display:flex !important;align-items:center;">@lang('User Dashboard')</a>
                    @endif

                </div>
            </div>
            <div class="ms-auto">
                <ul style="display: flex !important;" class="header-icons d-none d-lg-flex gap-3">
                    ...
                    @auth
                        {{-- profile --}}
                        <li class="user-icon d-none d-md-block" data-bs-toggle="tooltip" data-bs-placement="bottom"
...
{{--                            ace event login --}}
                            <a class="login-link d-none d-lg-block btn" href="{{ route('user.login',['login_type'=>$login_type??'normal']) }}"
...
@lang('Before you start bidding, please review and accept our') <a style="color: var(--primary-color) !important; text-decoration: underline !important;"  target="_blank" href="../../agreement/${data.data.event_id}">@lang('Bidder Agreement')</a> @lang('and') <a style="color: var(--primary-color) !important; text-decoration: underline !important;"  target="_blank" href="../../page/{{ get_policy_id() }}/terms-and-conditions">@lang('Terms and Conditions')</a>
    ...
    `);
        ...
        (function($) {
            ...
            setResponsiveImage(largeImageUrl, mediumImageUrl, smallImageUrl, document.getElementById('logo'), true);
            setResponsiveImage(largeImageUrl, mediumImageUrl, smallImageUrl, document.getElementById('logo-mobile'), true);
